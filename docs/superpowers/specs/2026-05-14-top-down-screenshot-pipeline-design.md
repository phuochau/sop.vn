...
**Context 2 — Frame chosen for a specific sub-step.** Used after Phase 2a returns `picked`. Signature: `selectInClusterFrame(cluster, searchWindow): DensePoolFrame` where `searchWindow = { start, end }` is computed at the call site from the sub-step's `narrationSegmentIds` or `timeWindow` per §2. Rule: among the chosen cluster's members, restrict to those whose timestamp falls inside `searchWindow`. If the in-window subset is non-empty, return the member whose dHash has the smallest Hamming distance to the cluster's centroid hash (i.e., the most "typical" appearance, restricted to the window). If the in-window subset is empty (cluster overlapped the window at its edges but its sampled members are outside), fall back to the cluster's centroid `representative`.
